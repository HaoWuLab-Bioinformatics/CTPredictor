...
def load_BOSIP_data(BOSIP, idX, Y):
    index = generate_bin()
...
class create_model(nn.Module):
    def __init__(self, model_para):
        ...
        linear_size_BOSIP_init = 2660
        linear_size_BCP_init = 2660
        linear_size_BOSP_init = 2660
        self.Con_layer_BOSIP, self.Con_layer_BCP, self.Con_layer_BOSP = Con_layer
        linear_size_BOSIP = com_linearsize(linear_size_BOSIP_init, self.Con_layer_BOSIP, kernel_size)
        ...
        self.linear_size_BOSIP = linear_size_BOSIP
        self.linear_size_BCP = linear_size_BCP
        self.linear_size_BOSP = linear_size_BOSP
        self.cnn_feature = cnn_feature  # 通道数
        if self.Con_layer_BOSIP != 0:
            self.conv1_BOSIP = nn.Conv1d(in_channels=1, out_channels=cnn_feature, kernel_size=kernel_size, stride=1,
                                       padding=1)
            self.bn1_BOSIP = nn.BatchNorm1d(num_features=cnn_feature)
            self.rule1_BOSIP = nn.ReLU()
            self.pool_BOSIP = nn.MaxPool1d(kernel_size=2)
            # self.dropout_BCP = nn.Dropout(dp)
            self.Con_BOSIP = nn.Sequential()
            for i in range(self.Con_layer_BOSIP - 1):
                layer_id = str(i + 2)
                self.Con_BOSIP.add_module("conv%s" % layer_id,
                                        nn.Conv1d(in_channels=cnn_feature, out_channels=cnn_feature,
                                                  kernel_size=kernel_size, stride=1, padding=1))
                self.Con_BOSIP.add_module("bach%s" % layer_id, nn.BatchNorm1d(num_features=cnn_feature))
                self.Con_BOSIP.add_module("relu%s" % layer_id, nn.ReLU())
                self.Con_BOSIP.add_module("maxp%s" % layer_id, nn.MaxPool1d(kernel_size=2))
        ...
        if linear_layer == 1:
            self.fc = nn.Linear(
                in_features=cnn_feature * (linear_size_BOSIP + linear_size_BCP + linear_size_BOSP),
                out_features=3)
        else:
            self.fc1 = nn.Linear(
                in_features=cnn_feature * (linear_size_BOSIP + linear_size_BCP + linear_size_BOSP ),
            ...
            self.fc2 = nn.Linear(in_features=out_feature, out_features=3)
        self.layer_1 = nn.LayerNorm(out_feature,eps=1e-6)
        self.lstm = nn.LSTM(input_size=out_feature,hidden_size=out_feature,num_layers=3,bidirectional=True)
        self.linear3 = nn.Sequential(
            nn.Linear(in_features=out_feature * 2 , out_features= out_feature),
            nn.ReLU(),
            nn.Dropout(dp)
        )
    ...
    def forward(self, x1, x2, x3):
        if self.Con_layer_BOSIP != 0:
            x1 = self.rule1_BOSIP(self.bn1_BOSIP(self.conv1_BOSIP(x1)))
            x1 = self.pool_BOSIP(x1)
            # x1 = self.dropout_BCP(x1)
            x1 = self.Con_BOSIP(x1)
            x1 = x1.view(-1, self.cnn_feature * self.linear_size_BOSIP)
        ...
        if self.Con_layer_BOSP != 0:
            x3 = self.rule1_BOSP(self.bn1_BOSP(self.conv1_BOSP(x3)))
...
def CNN_train(epoch, model, optimizer, train_loader, loss_fn, device):
    train_loader_BOSIP, train_loader_BCP, train_loader_BOSP = train_loader
    for (images_BOSIP, labels_BOSIP), (images_BCP, labels_BCP), (images_BOSP, labels_BOSP) \
            in zip(train_loader_BOSIP, train_loader_BCP, train_loader_BOSP):
        i = 0
        optimizer.zero_grad()
        labels = torch.Tensor(labels_BOSIP.type(torch.FloatTensor)).long()
        images_BOSIP = torch.unsqueeze(images_BOSIP.type(torch.FloatTensor), dim=1)
        ...
        # images_BOSIP = images_BOSIP.to(device)
        # images_BCP = images_BCP.to(device)
        # images_BOSP = images_BOSP.to(device)
        # labels = labels.to(device)
        outputs = model(images_BOSIP, images_BCP, images_BOSP)
        train_loss = loss_fn(outputs, labels)
        train_loss.backward()
        optimizer.step()
        train_loss += train_loss.cpu().data * images_BOSIP.size(0)
        _, prediction = torch.max(outputs.data, 1)
        print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
            epoch, i * len(images_BOSIP), len(train_loader_BOSIP.dataset),
                   100. * i / len(train_loader_BOSIP), train_loss.cpu().data * images_BOSIP.size(0)))
    return model, optimizer


def CNN_val(epoch, model, test_loader, loss_fn, test_size, device):
    test_loader_BOSIP, test_loader_BCP, test_loader_BOSP = test_loader
    i = 0
    for (images_BOSIP, labels_BOSIP), (images_BCP, labels_BCP), (images_BOSP, labels_BOSP) \
            in zip(test_loader_BOSIP, test_loader_BCP, test_loader_BOSP):
        images_BOSIP = torch.unsqueeze(images_BOSIP.type(torch.FloatTensor), dim=1)
        images_BCP = torch.unsqueeze(images_BCP.type(torch.FloatTensor), dim=1)
        images_BOSP = torch.unsqueeze(images_BOSP.type(torch.FloatTensor), dim=1)
        ...
        print('Val Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
            epoch, i * len(images_BOSIP), len(test_loader_BOSIP.dataset),
                   100. * i / len(test_loader_BOSIP), val_loss.cpu().data * images_BCP.size(0)))
        i = i + 1
    return label_pred, label, val_loss, model, val_accuracy


def CNN_1D_montage(BOSIP, BCP, BOSP, tr_x, tr_y, test_x, test_y, lr, model_para,alpha, gamma):
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    seed_torch()
    train_dataset_BOSIP, train_size_BOSIP = load_BOSIP_data(BOSIP, tr_x, tr_y)
    test_dataset_BOSIP, test_size_BOSIP = load_BOSIP_data(BOSIP, test_x, test_y)
    ...
    train_loader_BOSIP, test_loader_BOSIP = load_loader(train_dataset_BOSIP, test_dataset_BOSIP, test_size_BOSIP)
    ...
    print(model)
    # model.to(device)
    train_loader = [train_loader_BOSIP, train_loader_BCP, train_loader_BOSP]
    val_loader = [test_loader_BOSIP, test_loader_BCP, test_loader_BOSP]
    ...
    for epoch in range(num_epochs):
        ...
        test_label, label, test_loss, model, test_accuracy = CNN_val(epoch, model, val_loader,
                                                                     loss_fn, test_size_BOSIP, device)

    torch.save(model.state_dict(), 'shap_value/checkpoint_cpu.pt')
