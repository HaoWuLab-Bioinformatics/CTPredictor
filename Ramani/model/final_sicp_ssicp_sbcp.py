...
def load_Bosicp_dict():
    file_path = "../Ramani_features/bosicp/bosicp.npy"
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data


def load_Bosp_dict():
    file_path = '../Ramani_features/bin_linear_space_p/Bin_origin_space_p.npy'
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data

def load_Sbcp_dict():
    file_path = "../Ramani_features/sbcp/sbcp.npy"
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data

def load_Sicp_dict():
    file_path = "../Ramani_features/sicp/sicp.npy"
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data
...
def main():
    ...
    Con_layer = [Con_layer_SBCP,Con_layer_SICP,Con_layer_BOSICP]
    # linear_layer = 3
    # kernel_size = 7
    # cnn_feature = 32
    # out_feature = 256
    ...
    test_acc , test_label , real_label = CNN_1D_montage(SBCP,SICP,BOSICP,X_train,y_train,X_test,y_test,lr ,model_para,alpha,gamma)
