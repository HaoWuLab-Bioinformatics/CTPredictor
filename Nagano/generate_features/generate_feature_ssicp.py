...
def Small_Domain_Struct_Contact_pro(contact_matrix, index, scale):
    ...
    for i in range(index):
        ...
        if a == 0:
            BOSICP.append(float(0))
        else:
            BOSICP.append(float(a / chr_total))
    return BOSICP
...
def con_ran(cell_id, type, chr_name, max_length, cell_dict):
    ...
    file_path = "./Nagano/%s/%s_%s_%s.txt" % (type, type,str(cell_id), chr_name)
    ...
    Bosicp = Small_Domain_Struct_Contact_pro(contact_matrix, max_length, scale)

    return Bosicp


def main():
    types = {'early-S':451,'G1':234,'late-S_G2':443,'post-M':18,'pre-M':25} # 一共648个细胞
    resolutions = [1000000]
    tps = sorted(types)
    ...
    out_path = './features/bosicp/bosicp.npy'
