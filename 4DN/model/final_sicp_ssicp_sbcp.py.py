...
# from method_no_lstm import CNN_1D_montage
from sklearn.metrics import f1_score,precision_score
from collections import Counter

def load_Bosicp_dict():
    file_path = "../4DN_features/bosicp/bosicp.npy"
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data


def load_Sbcp_dict():
    file_path = "../4DN_features/sbcp/sbcp.npy"
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data


def load_Sbop_dict():
    file_path = '../4DN_features/bin_linear_space_p/Bin_origin_space_p.npy'
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data

def load_Sicp_dict():
    file_path = "../4DN_features/sicp/sicp.npy"
    Data = np.load(file_path, allow_pickle=True).item()  # 返回的长度为细胞数量
    return Data
