...
def Small_Domain_Struct_Contact_pro(contact_matrix, index, scale):
    ...
    for i in range(len(contact_matrix)):
        contact_information = []
        index1 = []
        contact_information.append(contact_matrix[i,:])
        for m in range(i - 1 , i + 2):
            if m < 0 or m > len(contact_matrix) - 1 or m == i:
                continue
            contact_information.append(contact_matrix[m,:])
            index1.append(m)
        for n in range(len(contact_matrix[0])):
            if n in index1:
                continue
            if contact_matrix[i,n] != 0 and i != n:
                contact_information.append(contact_matrix[n,:])
        matrix1[i,:] = smooth_mean(contact_information)
    m = np.triu(matrix1,1)
    n = m.T
    final_matrix = m + n + np.diag(np.diag(matrix1))
    # 计算单个细胞中单个染色质的染色质接触概率

    contact_matrix = np.array(final_matrix)  # 染色体的接触矩阵
    ...
    for i in range(index):
        ...
        if a == 0:
            SSICP.append(float(0))
        else:
            SSICP.append(float(a / chr_total))
    return SSICP
...
def con_ran(cell_id, type, chr_name, max_length, cell_dict):
    ...
    file_path = "../Data_filter/4DN/%s/cell_%s_%s.txt" % (type, str(cell_id), chr_name)
    ...
    Ssicp = Small_Domain_Struct_Contact_pro(contact_matrix, max_length, scale)

    return Ssicp


def main():
    types = {'GM12878':1591,'H1Esc':1240,'HAP1':916,'HFF':356,'IMR90':12} # 一共648个细胞
    resolutions = [1000000]
    tps = sorted(types)
    ...
    for type in tps:
        for c_num in range(types[type]):
            ...
            for chr in chr_list:
                ...
                # print(args)
                Ssicp = con_ran(cell_id, type, chr, max_len, cell_dict)
                cell_dict[str(cell_number)][chr] = Ssicp
    out_path = './4DN_features/bosicp/bosicp.npy'
